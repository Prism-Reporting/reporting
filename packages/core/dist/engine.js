 ...
 */
export function validateReportSpec(spec, context = {}) {
    const diagnostics = [];
    ...
    if (!isNonEmptyString(rawSpec.layout)) {
        ...
        addError("layout", "invalid-layout", `ReportSpec.layout "${rawSpec.layout}" is not supported`, 'Use "singleColumn" or "twoColumn".');
    }
    ...
    for (const [index, filter] of filters.entries()) {
        ...
        if (!isRecord(filter)) {
            addError(path, "invalid-filter", `Filter at index ${index} must be an object`, "Provide a valid filter definition.");
            continue;
        }
        const id = filter.id;
        if (!isNonEmptyString(id)) {
            ...
            addError(`${path}.id`, "duplicate-filter-id", `Duplicate filter id "${id}"`, "Ensure every filter id is unique within the report.");
        }
        else {
            filterIds.add(id);
        }
        ...
        if (!isNonEmptyString(filter.dataSource)) {
            addError(`${path}.dataSource`, "missing-filter-data-source", `Filter "${String(id ?? index)}" must reference a dataSource`, "Use the id of one of the dataSources defined in the spec.");
        }
        else if (!dataSourceNames.has(filter.dataSource)) {
            addError(`${path}.dataSource`, "unknown-filter-data-source", `Filter "${String(id ?? index)}" references unknown dataSource "${filter.dataSource}"`, "Use a dataSource key that exists in ReportSpec.dataSources.");
        }
        switch (filter.type) {
            case "select": {
                if (!Array.isArray(filter.options) || filter.options.length === 0) {
                    addError(`${path}.options`, "invalid-select-options", `Select filter "${String(id ?? index)}" must define at least one option`, "Provide one or more { value, label } options.");
                    break;
                }
            ...
            case "search":
                break;
            default:
                addError(`${path}.type`, "unsupported-filter-type", `Filter "${String(id ?? index)}" uses unsupported type "${String(filter.type)}"`, 'Use one of "select", "dateRange", or "search".');
        }
    }
    ...
    for (const [index, widget] of widgets.entries()) {
        ...
        if (!isRecord(widget)) {
            addError(path, "invalid-widget", `Widget at index ${index} must be an object`, "Provide a valid widget definition.");
            continue;
        }
        const id = widget.id;
        if (!isNonEmptyString(id)) {
            ...
            addError(`${path}.id`, "duplicate-widget-id", `Duplicate widget id "${id}"`, "Ensure every widget id is unique within the report.");
        }
        else {
            widgetIds.add(id);
        }
        if (!isNonEmptyString(widget.dataSource)) {
            addError(`${path}.dataSource`, "missing-widget-data-source", `Widget "${String(id ?? index)}" must reference a dataSource`, "Use the id of one of the dataSources defined in the spec.");
        }
        else if (!dataSourceNames.has(widget.dataSource)) {
            addError(`${path}.dataSource`, "unknown-widget-data-source", `Widget "${String(id ?? index)}" references unknown dataSource "${widget.dataSource}"`, "Use a dataSource key that exists in ReportSpec.dataSources.");
        }
        if (!isRecord(widget.config)) {
            addError(`${path}.config`, "missing-widget-config", `Widget "${String(id ?? index)}" must define a config object`, "Provide the config required for the widget type.");
            continue;
        }
        ...
        switch (widget.type) {
            case "table": {
                if (!Array.isArray(widget.config.columns) || widget.config.columns.length === 0) {
                    addError(`${path}.config.columns`, "invalid-table-columns", `Table widget "${String(id ?? index)}" must define at least one column`, "Provide one or more { key, label } column definitions.");
                    break;
                }
            ...
            case "kpi":
                ...
                break;
            default:
                addError(`${path}.type`, "unsupported-widget-type", `Widget "${String(id ?? index)}" uses unsupported type "${String(widget.type)}"`, 'Use one of "table", "barChart", or "kpi".');
        }
    }
    return {
        version: REPORT_SPEC_VERSION,
